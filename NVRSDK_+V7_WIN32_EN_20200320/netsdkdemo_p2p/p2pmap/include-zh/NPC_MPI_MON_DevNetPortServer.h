...
IN		PNPC_VOID							in_pPortData,										//[IN]端口数据
IN		PNPC_BYTE							in_pDataBuf,                                        //[IN]数据缓冲区
IN		NPC_INT								in_iDataLen                                         //[IN]数据长度
);
...
NPC_MPI_MON_DNP_API		NPC_INT	NPC_F_MPI_MON_DNP_GetBestSrv(
	...
	OUT		PNPC_CHAR							out_pBestSrvIp,				//[OUT]最优服务器IP
	OUT		PNPC_INT							out_pBestSrvPotr			//[OUT]最优服务器端口
	);
#endif
...
NPC_MPI_MON_DNP_API		NPC_HANDLE	NPC_F_MPI_MON_DNP_CreateDevNetPortServer(
    ...
    IN      NPC_BOOL                            in_bIfConnSpecSrv=TRUE      //[IN]是否连接指定服
	);
...
NPC_MPI_MON_DNP_API		NPC_VOID		NPC_F_MPI_MON_DNP_DisconnectDevAtCloud(
    IN		NPC_HANDLE							in_hDevNetPort,				//[IN]设备网络端口
    IN		NPC_HANDLE							in_hPortMapData				//[IN]端口映射数据
    );
...
NPC_MPI_MON_DNP_API		NPC_INT		NPC_F_MPI_MON_DNP_SetPrivateTrans(
	IN		NPC_HANDLE							in_hDevNetPort,				//[IN]设备网络端口
	IN		NPC_BOOL							in_bIfUsePriTrans=FALSE,	//[IN]是否使用私有转发服务器
	IN		NPC_DWORD							in_dwEnterId=0				//[IN]企业ID
	);
...
NPC_MPI_MON_DNP_API		NPC_BOOL	NPC_F_MPI_MON_DNP_SendData(
    IN		NPC_HANDLE							in_hPortMapData,			//[IN]端口映射数据
    IN		PNPC_CHAR							in_pSendData,				//[IN]发送的数据缓冲区
    IN		NPC_INT                             in_iDataLen,				//[IN]发送的数据长度
    IN		NPC_INT								in_iTimeout					//[IN]超时时长（毫秒）
    );
//接收数据包
NPC_MPI_MON_DNP_API		NPC_BOOL	NPC_F_MPI_MON_DNP_GetData(
    IN		NPC_HANDLE							in_hPortMapData,			//[IN]端口映射数据
    OUT		PNPC_CHAR							out_iDataBuf,               //数据缓冲区
    OUT		PNPC_INT							out_iDataLen,               //数据长度
    IN		NPC_INT								in_iTimeout					//[IN]超时时长（毫秒）
    );
//----------------------------------------------------------------------------------------
...
//接收数据回调，Callback里面的用户数据为NPC_F_MPI_MON_DNP_ConnDevAtCloud接口返回的设备端口数据句柄
NPC_MPI_MON_DNP_API		NPC_VOID	NPC_F_MPI_MON_DNP_SetP2pPortDataCallback(
    IN      PNPC_F_MPI_MON_CALLBACK_DevNetPortP2pPortData   in_pCallback,       //[IN]回调
    IN		PNPC_VOID                                   in_hUserData,		//[IN]用户数据
    IN		NPC_HANDLE                                  in_hPortMapData     //[IN]端口映射数据
    );
...
NPC_MPI_MON_DNP_API		NPC_VOID	NPC_F_MPI_MON_DNP_Dev_PublicIpAddr(
    IN		NPC_HANDLE							in_hPortMapData,			//[IN]端口映射数据
    OUT		PNPC_BYTE*							out_iDataBuf,               //数据缓冲区
    OUT		PNPC_INT							out_iDataLen                //数据长度
    );
// C++ support
#ifdef __cplusplus
