...
struct TDecRecDownload
{
	s32 nChnId;                                        ///< 通道ID
	s32 nAudioChnNum;                                  ///< 音频通道数
	s8 szRecFileName[DEC_MAXLEN_256];                  ///< 录像文件名，全称：路径+文件名
	TDecRtspParam tRtspParam;                          ///< rtsp参数
};
...
/**
 * @brief  开始录像下载
 * @param  tRecDownload      [in]录像下载参数
 *         pdwHandle         [out]录像下载句柄，从1开始
 * @return 成功返回DEC_OK，失败返回错误码
 */
DEC_API s32 STDCALL DEC_StartRecDownloadTcp(TDecRecDownload tRecDownload, u32 *pdwHandle);

/**
 * @brief  停止录像下载
 * @param  dwHandle      [in]录像下载句柄
 * @return 成功返回DEC_OK，失败返回错误码
 */
DEC_API s32 STDCALL DEC_StopRecDownload(u32 dwHandle);
